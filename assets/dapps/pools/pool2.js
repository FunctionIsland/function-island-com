...
async function run() {
    poolAddress = 'TKEcUzeTxcf9GBohR6kUSP3MDvArNabc4v'; // Function Island staking pool
    funcAddress = 'TM2C3xvyz8bDbG96RyET82YEAKdcxoVcV2'; // FUNC Token - what you'll receive for staking.
    stknAddress = 'TM2C3xvyz8bDbG96RyET82YEAKdcxoVcV2'; // LPTOKEN Token - what you'll stake to get FUNC...
    ...
    function init() {
        ...
        $('#approveButton').click(function () {
            LPTOKEN.approve(poolAddress, "10000000000000000000000000").send(function (error, hash) {
                if (!error) {
    ...
    function update() {
        ...
        LPTOKEN.allowance(account, poolAddress).call(function (error, result) {
            if (!error) {
                _userLPTokenAllowance = result;

                if (_userLPTokenAllowance < _lpTokenTotalSupply) {
                    // Interface hide
                    $('#stakeBox').hide();
                    $('#approveButton').show();
        ...
        TKNPOOL.totalSupply().call(function (error, result) {
            if (!error) {
                ...
                _totalStaked = result;
                _apy = ((_rawRewardRate * _tokenPrice * 31536000) / _totalStaked).toFixed(2);
